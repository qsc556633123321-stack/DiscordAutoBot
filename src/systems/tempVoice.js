...
function buildControlRows(channelId) {
  return [
...
async function sendTempVoicePanel({ guild, channel, record }) {
  const settings = getTempVoiceSettings(guild.id);
  if (!settings.createActivityMessage && !settings.createControlPanel) return;
  ...
  try {
    const patch = {
      ...
      textControlChannelId: targetChannel.id
    };

    if (settings.createActivityMessage) {
      const activityEmbed = new EmbedBuilder()
        .setColor(0x57f287)
        .setTitle(`🎮 ${record.game} 建立了語音房`)
        .setDescription(`${channel}\n目前人數：${Math.max(channel.members.size, 1)}`)
        .setTimestamp();

      const activityMessage = await targetChannel.send({ embeds: [activityEmbed] });
      patch.activityMessageId = activityMessage.id;
      setTimeout(() => {
        activityMessage.delete().catch(() => null);
      }, 10 * 60 * 1000);
    }

    if (settings.createControlPanel) {
      ...
      patch.controlMessageId = controlMessage.id;
    }

    updateTempVoiceRecord(guild.id, channel.id, patch);
  } catch (error) {
    console.error('發送 temp voice 控制台失敗：', error);
  }
}
...
function getTempVoiceChannelFromInteraction(interaction, channelId) {
  ...
  if (!isTempVoice(interaction.guild.id, channel.id)) return null;
  return channel;
}

async function assertTempVoiceControl(interaction, channelId) {
  if (!interaction.guild) {
    await interaction.reply({ content: '這個操作只能在伺服器內使用。', ephemeral: true });
    return null;
  }

  const channel = getTempVoiceChannelFromInteraction(interaction, channelId);
  if (!channel) {
    await interaction.reply({ content: '找不到可控制的臨時語音房。', ephemeral: true });
    return null;
  }

  const record = getTempVoiceRecord(interaction.guild.id, channel.id);
  if (!record || !canControlTempVoice(interaction, record)) {
    await interaction.reply({ content: '只有房主或管理員可以控制這個房間。', ephemeral: true });
    return null;
  }
...
async function handleTempVoiceButton(interaction) {
  ...
  if (id.startsWith('tempvoice_lock_')) {
    ...
    if (!context) return;
    await context.channel.permissionOverwrites.edit(interaction.guild.roles.everyone.id, { Connect: false }, { reason: 'Temp voice locked by owner' });
    updateTempVoiceRecord(interaction.guild.id, context.channel.id, { locked: true });
    await interaction.reply({ content: '🔒 房間已鎖定。', ephemeral: true });
    return;
  }
  ...
  if (id.startsWith('tempvoice_open_')) {
    ...
    if (!context) return;
    await context.channel.permissionOverwrites.edit(interaction.guild.roles.everyone.id, { Connect: true }, { reason: 'Temp voice opened by owner' });
    updateTempVoiceRecord(interaction.guild.id, context.channel.id, { locked: false });
    await interaction.reply({ content: '🌐 房間已公開。', ephemeral: true });
    return;
  }
  ...
  if (id.startsWith('tempvoice_limit_')) {
    ...
    await interaction.reply({ content: '請選擇房間人數限制。', components: [row], ephemeral: true });
    return;
  }
  ...
  if (id.startsWith('tempvoice_transfer_')) {
    ...
    if (!members.length) {
      await interaction.reply({ content: '房內目前沒有可以移交的成員。', ephemeral: true });
      return;
    }
    const row = new ActionRowBuilder().addComponents(
      new StringSelectMenuBuilder()
    ...
    await interaction.reply({ content: '請選擇新的房主。', components: [row], ephemeral: true });
    return;
  }

  if (id.startsWith('tempvoice_disband_cancel_')) {
    await interaction.reply({ content: '已取消解散房間。', ephemeral: true });
    return;
  }
  ...
  if (id.startsWith('tempvoice_disband_confirm_')) {
    ...
    if (!context) return;
    await interaction.reply({ content: '正在解散房間...', ephemeral: true });
    ...
    removeTempVoice(interaction.guild.id, context.channel.id);
    return;
  }
  ...
  if (id.startsWith('tempvoice_disband_')) {
    ...
    await interaction.reply({ content: `確定要解散 ${context.channel} 嗎？`, components: [row], ephemeral: true });
  }
}
...
async function handleTempVoiceSelect(interaction) {
  if (interaction.customId.startsWith('tempvoice_limit_select_')) {
    ...
    updateTempVoiceRecord(interaction.guild.id, context.channel.id, { userLimit: limit || 0 });
    await interaction.reply({ content: `👥 人數限制已更新為：${limit || '無限制'}`, ephemeral: true });
    return true;
  }
  ...
  if (interaction.customId.startsWith('tempvoice_transfer_select_')) {
    ...
    updateTempVoiceRecord(interaction.guild.id, context.channel.id, { ownerId: nextOwnerId });
    await interaction.reply({ content: `👑 已轉移房主給 <@${nextOwnerId}>`, ephemeral: true });
    return true;
  }
...
async function handleTempVoiceModal(interaction) {
  ...
  if (!name) {
    await interaction.reply({ content: '名稱不可為空，且不能包含 @everyone、@here 或 discord.gg。', ephemeral: true });
    return true;
  }

  await context.channel.setName(`🔊｜${name}`, 'Temp voice renamed by owner');
  await interaction.reply({ content: `✏️ 房間已改名為：🔊｜${name}`, ephemeral: true });
  return true;
}
