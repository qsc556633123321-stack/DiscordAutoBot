...
module.exports = {
  ...
  async execute(interaction) {
    ...
    try {
      const rules = listChannelRules(interaction.guild.id).slice(0, 25);
      const description = rules.length
        ? rules.map((rule, index) => (
          `**${index + 1}.** \`${rule.keyword}\` -> \`${rule.category}\`（+${rule.weight}）`
        )).join('\n')
        : '目前尚未學習任何頻道分類規則。';
      ...
      console.error('讀取記憶規則失敗：', error);
      await interaction.reply({
        content: `讀取記憶規則失敗：${error.message}`,
        ephemeral: true
      });
    }
  }
