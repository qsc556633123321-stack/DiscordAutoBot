...
| Migrated read query | `help-me-start` legacy command, interactive guide compatibility helper, Domain/Application/Composition/Presentation slice | Recommendation business rules now have one active Domain source; the Concierge text generator remains a narrow compatibility adapter. | Keep command and helper wrappers during the observation window; do not move Guide setup or Concierge buttons. |
