...
| 2 | Guide content/status query | Concierge helpers | **Complete:** focused read composition and compatibility payload delegation | `getOrCreate`, publication, roles remain excluded | content/status/stale record fixtures | Medium; interaction fallback | no mutation path changes |
