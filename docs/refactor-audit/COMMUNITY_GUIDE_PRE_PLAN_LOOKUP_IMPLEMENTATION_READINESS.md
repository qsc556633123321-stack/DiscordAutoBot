...
| B. Production Application Lookup Port + test fake | Ready with explicit exclusions | Recommended next slice; no runtime wiring |
| C. Infrastructure lookup adapter preparation | Needs more preparation | Requires port implementation baseline |
...
The recommended next slice is **B**, restricted to an Application port contract
and test fake. It must not create a production Discord adapter, composition
