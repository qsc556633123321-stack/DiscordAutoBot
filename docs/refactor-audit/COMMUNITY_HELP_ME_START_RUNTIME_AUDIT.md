...
| Legacy helper | `interactiveGuideSystem.buildHelpMeStartEmbed` had the only discovered runtime consumer: legacy command | Retained as compatibility consumer; delegates Composition only and does not import Presentation or own recommendation/rendering rules. |
...
aliasRegistry -> legacy help-me-start thin wrapper
  ...
  -> Discord guild-channel reader + `adapters/legacy` Concierge text adapter
  -> Presentation EmbedBuilder / deferred ephemeral editReply
```

The original migration's Infrastructure-to-System and System-to-Presentation reverse dependencies were removed in the cleanup pass. No command registry, alias registry, deploy script, Concierge button, Guide setup, or Roadmap path changed.
