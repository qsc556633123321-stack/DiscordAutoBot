...
(async () => {
  ...
  const message = { id: 'M', async edit(input) { calls.edit += 1; assert.strictEqual(input, payload); return { id: 'OTHER' }; } };
  const sent = { id: 'S' };
  ...
  console.log('Roadmap production mutation adapter integrates with the production Session');
