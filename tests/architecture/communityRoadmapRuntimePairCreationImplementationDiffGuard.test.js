...
assert.match(roadmap, /const channel = await getOrCreateRoadmapChannel\(guild\);\s+const \{ lookupPort, getRetainedMessage \} =\s+communityRoadmapAdapterPairFeature\.createAdapterPair\(\{ ensuredChannel: channel \}\);/s);
...
assert.doesNotMatch(roadmap, /mutationPort/);
