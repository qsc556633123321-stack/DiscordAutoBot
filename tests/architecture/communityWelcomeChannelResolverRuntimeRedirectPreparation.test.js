...
assert.equal(welcome.includes('channels.cache.get'), true);
assert.equal(welcome.includes('channels.fetch'), true);
assert.equal(welcome.includes('findChannelByName(member.guild, GUIDE_CHANNEL_NAME)'), true);
assert.equal(welcome.includes('createCommunityWelcomeChannelResolver'), false);
assert.equal(welcome.includes('channelResolver.resolve'), false);
...
console.log('Welcome resolver runtime redirect preparation keeps production runtime, resolver, tracking, filesystem, and DM ownership frozen.');
