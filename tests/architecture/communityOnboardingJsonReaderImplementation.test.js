...
assert.equal((runtime.match(/createCommunityOnboardingJsonReader\(/g) || []).length, 0, 'runtime does not construct explicit-path readers');
assert.equal((runtime.match(/createDefaultCommunityOnboardingJsonReader\(\)/g) || []).length, 3, 'runtime constructs one default reader per closed flow');
