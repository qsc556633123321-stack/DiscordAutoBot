...
保護規則：不刪 ticket- 頻道、不刪臨時語音、不刪執行指令頻道；一般聊天、深夜聊天、找隊友大廳、認真討論與 `apex-`、`tft-`、`lol-`、`mc-`、`特戰-` 開頭頻道視為有效頻道。美食、音樂、迷因、活動類頻道會先列入合併/封存候選，不會 preview 後直接執行。
...
為了減少相似頻道，`迷因與好圖`、`音樂分享`、`美食分享`、`活動公告`、`投票區`、`比賽與排行` 不再是標準必建頻道。執行 layout repair preview 時，這些頻道會被列為合併/封存候選；有訊息的會建議先移到 `📦｜舊頻道封存`，空頻道或測試頻道才會列為刪除候選。

遊戲提議入口：

- `📋｜遊戲提議` 會有「🎮 想新增遊戲區嗎？」面板。
- 使用者按「🎮 提議新遊戲」會直接跳出 Modal 表單，不必記 `/suggest-game`。
- Modal 會詢問遊戲名稱、提議理由，以及希望建立哪些內容。
- 按「📋 查看目前提議」會列出待審核提議。
- 按「🧭 查看已有遊戲區」會列出目前存在的 `🎮｜遊戲名稱` 分類。
- 管理員按「✅ 批准」後會自動建立遊戲分類、聊天、找隊友、資訊與建立語音入口，並同步 Temp Voice metadata、Voice Hub 與遊戲面板。
- 管理員按「❌ 拒絕」會跳出 Modal 填寫拒絕理由，提議卡會更新為已拒絕。
